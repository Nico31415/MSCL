...
class Model(nn.Module):
    def __init__(self, img_size, latent_dim, **kwargs):
        ...
        if list(img_size[1:]) not in [[32, 32], [64, 64]]:
            raise RuntimeError(
                "{} sized images not supported. Only (None, 32, 32) and (None, 64, 64) supported. Build your own architecture or reshape images!"
                .format(img_size))
        ...
        self.dist_nparams = 2
        self.encoder = Encoder(img_size, self.latent_dim, dist_nparams=self.dist_nparams)
